...
namespace VykazyPrace.Dialogs
{
    public partial class ExportDialog : Form
    {
        ...
        private async Task LoadTimeEntriesSummaryAsync(DateTime fromDate, DateTime toDate)
        {
            ...
            var summaryListResult = await _timeEntryRepo.GetTimeEntriesSummaryAsync(fromDate, toDate);
            if (!summaryListResult.Success)
                return;

            var summaries = summaryListResult.Summaries;

            var data = await Task.WhenAll(summaries.Select(async summary =>
            {
                var userResult = await _userRepo.GetUserByIdAsync((int)summary.UserId);
                if (!userResult.Success) return null;

                var projectResult = await _projectRepo.GetProjectByIdAsync((int)summary.ProjectId);
            ...
            UpdateDataGrid(data.Where(d => d != null).ToList());
        }
        ...
        private async Task InitializeUsers()
        {
            ...
            {
                UpdateCheckedListBox(usersResult.Users);
            }
        ...
        private async Task ExportToExcel(string filePath)
        {
            ...
            try
            {
                ...
                var timeEntriesResult = await _timeEntryRepo.GetAllTimeEntriesAsync();
                if (timeEntriesResult.Success)
                {
                    var timeEntries = timeEntriesResult.Entries;
                ...
                {
                    AppLogger.Error("Nepodařilo se načíst časové záznamy.", new Exception(timeEntriesResult.ErrorMessage));
                }
            }
